...
`Config` covers `maxima_root`, `installation`, `search`, `timeout`, `startup_timeout`, `cache_entries`,
...
Which Maxima is used: `Config::installation`, which names the SBCL
executable and the Maxima core together and ends the search; else
`Config::maxima_root`, then
`$MAXIMA_ROOT`, `$MAXIMA_PREFIX`, the parent of any `$PATH` entry named
