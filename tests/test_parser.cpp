...
/// True when `source` is refused with Cause::Parse and a message mentioning
...
TEST_CASE("a number beyond a double's range is refused, and says so") {
    ...
    for (const char *source : {"1e400", "-1e400", "2*1e999"}) {
        CAPTURE(source);
        CHECK(refused(source, "out of the range of a double"));
    }
...
TEST_CASE("malformed input is rejected, with the offset") {
    ...
    CHECK(refused("f(x"));
    CHECK(refused("[1, 2"));
    CHECK(refused(R"("unterminated)"));
    CHECK(refused("x @ y"));
    // Two expressions where one was promised.
    CHECK(refused("1 2"));

    SUBCASE("and the message says where") {
        CHECK(refused("x + @", "offset 4"));
        // And unwrap throws it as the exception the cause names, for a
        // caller who would rather catch.
        CHECK_THROWS_WITH_AS(proxima::unwrap(Expr::parse("x + @")),
                             doctest::Contains("offset 4"), proxima::ParseError);
    }
}
...
TEST_CASE("text nested too deep is refused, not a stack overflow") {
    ...
    for (const std::string *source :
        ...
        CHECK(refused(*source, "nested too deep"));
    }
...
TEST_CASE("statements are not expressions, and are refused") {
    ...
    CHECK(refused("a: 7"));
    CHECK(refused("f(x) := x^2"));
    CHECK(refused("'diff(f(x), x)"));
}
