...
TEST_CASE("numeric leaves evaluate to themselves") {
    CHECK(proxima::evalNumeric(Expr(42)) == doctest::Approx(42.0));
    CHECK(proxima::evalNumeric(Expr(-7)) == doctest::Approx(-7.0));
    CHECK(proxima::evalNumeric(Expr(1.5)) == doctest::Approx(1.5));
    CHECK(proxima::evalNumeric(Expr::rational(1, 4)) == doctest::Approx(0.25));
}
...
TEST_CASE("symbols take their value from the bindings") {
    ...
    CHECK(proxima::evalNumeric(Expr(x), {{"x", 3.0}}) == doctest::Approx(3.0));
    CHECK(proxima::evalNumeric(Expr(x) * Expr(y), {{"x", 3.0}, {"y", 4.0}})
          == doctest::Approx(12.0));
}
...
TEST_CASE("arithmetic") {
    ...
    CHECK(proxima::evalNumeric(pow(Expr(x), 2) + 3 * Expr(x) + 2, at2)
          == doctest::Approx(12.0));
    CHECK(proxima::evalNumeric(Expr(1) / Expr(x), at2) == doctest::Approx(0.5));
    CHECK(proxima::evalNumeric(-Expr(x), at2) == doctest::Approx(-2.0));
    CHECK(proxima::evalNumeric(proxima::sqrt(Expr(x)), at2)
...
TEST_CASE("the functions Maxima leaves in a result") {
    ...
    CHECK(proxima::evalNumeric(proxima::sin(Expr(x)), at1) == doctest::Approx(std::sin(1.0)));
    CHECK(proxima::evalNumeric(proxima::cos(Expr(x)), at1) == doctest::Approx(std::cos(1.0)));
    CHECK(proxima::evalNumeric(proxima::log(Expr(x)), at1) == doctest::Approx(0.0));
    CHECK(proxima::evalNumeric(proxima::abs(-Expr(x)), at1) == doctest::Approx(1.0));
    CHECK(proxima::evalNumeric(proxima::exp(Expr(x)), at1) == doctest::Approx(std::numbers::e));
    CHECK(proxima::evalNumeric(Expr::function("max", {Expr(1), Expr(5), Expr(3)}))
          == doctest::Approx(5.0));
}
...
TEST_CASE("a long call and a refused expression evaluate as before") {
    ...
    CHECK(proxima::evalNumeric(Expr::function("max", many), {{"x", 2.0}}) == 24.0);
    CHECK(proxima::evalNumeric(Expr::function("min", many), {{"x", 2.0}}) == 2.0);
    CHECK(proxima::isEvaluable(Expr::function("max", many), {{"x", 2.0}}));

    CHECK_FALSE(proxima::isEvaluable(Expr(x) + 1));
    CHECK_FALSE(proxima::isEvaluable(eq(Expr(x), Expr(1)), {{"x", 1.0}}));
    CHECK_FALSE(proxima::isEvaluable(Expr::opaque("matrix([1])")));
    CHECK_FALSE(proxima::isEvaluable(Expr::function("no_such_function", {Expr(1)})));

    // And evalNumeric still says why.
    CHECK_THROWS_WITH_AS(proxima::evalNumeric(Expr(x) + 1), "no value for the symbol x",
                         proxima::EvalError);
}
...
TEST_CASE("the compiled form keeps 0.0 and -0.0 apart") {
    ...
    const double walked = proxima::evalNumeric(signs, {{"x", 0.0}});
    ...
    SUBCASE("and so does a reciprocal") {
        ...
        CHECK(std::isnan(proxima::evalNumeric(reciprocals, {{"x", 1.0}})));
...
TEST_CASE("bindings name a symbol by the Symbol or by its name") {
    ...
    CHECK(proxima::asFunction(Expr(x) * Expr(y), x, {{y, 2.0}})(3.0) == doctest::Approx(6.0));
    CHECK(proxima::isEvaluable(Expr(x), {{x, 1.0}}));
    ...
    CHECK(proxima::evalNumeric(Expr(x) + Expr(y), bindings) == doctest::Approx(7.0));
...
TEST_CASE("constants are recognised as Maxima spells them") {
    CHECK(proxima::evalNumeric(proxima::pi()) == doctest::Approx(std::numbers::pi));
    CHECK(proxima::evalNumeric(proxima::e()) == doctest::Approx(std::numbers::e));
    CHECK(std::isinf(proxima::evalNumeric(proxima::inf())));
    CHECK(proxima::evalNumeric(proxima::minf()) < 0);
    CHECK(proxima::evalNumeric(proxima::Expr::symbol("%phi")) == doctest::Approx(std::numbers::phi));
    CHECK(proxima::evalNumeric(proxima::Expr::symbol("%gamma"))
    ...
    SUBCASE("but an explicit binding still wins") {
        ...
        CHECK(proxima::evalNumeric(proxima::e(), {{"%e", 10.0}}) == doctest::Approx(10.0));
    }
}
...
TEST_CASE("what cannot be evaluated says so rather than guessing") {
    ...
    SUBCASE("an unbound symbol") {
        CHECK_THROWS_AS(proxima::evalNumeric(Expr(x)), proxima::EvalError);
    }
    SUBCASE("a function with no numeric meaning here") {
        ...
        CHECK_THROWS_AS(
            proxima::evalNumeric(Expr::function("bessel_j", {Expr(0), Expr(1)})),
            proxima::EvalError);
    }
    SUBCASE("a relation") {
        CHECK_THROWS_AS(proxima::evalNumeric(eq(Expr(1), Expr(1))), proxima::EvalError);
    }
    SUBCASE("an Opaque node") {
        // Maxima source this library never interpreted, so there is nothing
        // here that could evaluate it.
        CHECK_THROWS_AS(proxima::evalNumeric(Expr::opaque("30!")), proxima::EvalError);
    }
    SUBCASE("and the message names the culprit") {
        try {
            proxima::evalNumeric(Expr(x) + 1);
...
TEST_CASE("isEvaluable answers without throwing") {
    const Symbol x("x");
    CHECK_FALSE(proxima::isEvaluable(Expr(x)));
    CHECK(proxima::isEvaluable(Expr(x), {{"x", 1.0}}));
    CHECK(proxima::isEvaluable(proxima::pi()));
    CHECK_FALSE(proxima::isEvaluable(Expr::opaque("30!")));
}

TEST_CASE("asFunction binds one variable for repeated use") {
    const Symbol x("x");
    const auto f = proxima::asFunction(pow(Expr(x), 2), x);
    ...
    SUBCASE("with the other symbols fixed") {
        const Symbol a("a");
        const auto scaled = proxima::asFunction(Expr(a) * Expr(x), x, {{"a", 10.0}});
...
TEST_CASE("a compiled expression agrees with the one-shot evaluator") {
    ...
    for (const char *source : {
        ...
        for (const double at : {-2.5, -1.0, -0.25, 0.0, 0.5, 1.0, 3.75}) {
            CAPTURE(at);
            CHECK(sameNumber(compiled(at), proxima::evalNumeric(f, {{"x", at}})));
        }
    }
}
...
TEST_CASE("a result from Maxima can be evaluated without another round trip") {
    ...
    CHECK(proxima::evalNumeric(*antiderivative, {{"x", 1.0}})
...
TEST_CASE("a definite integral agrees with sampling its antiderivative") {
    ...
    CHECK(f(1.0) - f(0.0) == doctest::Approx(proxima::evalNumeric(*exact)));
}
...
TEST_CASE("mod and round are Maxima's, not the C library's") {
    ...
    const auto mod = [](double a, double b) {
        return proxima::evalNumeric(Expr::function("mod", {Expr(a), Expr(b)}));
    };
    const auto round = [](double value) {
        return proxima::evalNumeric(Expr::function("round", {Expr(value)}));
    };
    ...
    SUBCASE("infinities and NaN pass through") {
        ...
        const Symbol x("x");
        CHECK(std::isnan(proxima::evalNumeric(Expr::function("round", {Expr(x)}),
                                         {{"x", std::nan("")}})));
    }

    SUBCASE("the compiled form agrees, since it shares the function table") {
        const Symbol x("x");
        const proxima::Compiled compiledMod(Expr::function("mod", {Expr(x), Expr(3)}),
                                       x);
        CHECK(compiledMod(-7.0) == 2.0);
        CHECK(compiledMod(-6.0) == 0.0);
        const proxima::Compiled compiledRound(Expr::function("round", {Expr(x)}), x);
...
TEST_CASE("mod and round agree with Maxima across signs and halves") {
    ...
    const auto maxima = [&kernel](const std::string &source) {
        ...
        return proxima::evalNumeric(*answer);
    };
    ...
    for (const double a : dividends) {
        for (const double b : divisors) {
            ...
            CAPTURE(source);
            const double local
                = proxima::evalNumeric(Expr::function("mod", {Expr(a), Expr(b)}));
    ...
    for (const double value : values) {
        ...
        CAPTURE(source);
        const double local
            = proxima::evalNumeric(Expr::function("round", {Expr(value)}));
        CHECK(sameNumber(local, maxima(source)));
    }
}
