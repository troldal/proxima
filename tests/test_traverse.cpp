...
TEST_CASE("replace rewrites a symbol locally, and the result is normalised") {
    ...
    CHECK(proxima::replace(Expr(x) * Expr(y), x, Expr(y) + 1)
          == (Expr(y) + 1) * Expr(y));
    CHECK(proxima::replace(eq(Expr(x), Expr(y)), x, Expr(2))
          == eq(Expr(2), Expr(y)));
    ...
    SUBCASE("but not evaluated, which is Maxima's work") {
        CHECK(proxima::replace(proxima::sin(Expr(x)), x, Expr(0))
              == proxima::sin(Expr(0)));
        CHECK(proxima::replace(pow(Expr(x), 2), x, Expr(2))
              == pow(Expr(2), Expr(2)));
    }
    ...
    SUBCASE("unmodelled text that mentions the symbol is refused, not skipped") {
        CHECK_THROWS_AS(
            proxima::replace(Expr::opaque("matrix([x])") + Expr(x), x, Expr(1)),
            proxima::Error);
    ...
    SUBCASE("and it composes with the numeric layer") {
        ...
        CHECK(
            fixed(3.0)
            == doctest::Approx(*proxima::eval_numeric(f, {{"a", 2.5}, {"x", 3.0}})));
    }
}
...
TEST_CASE("visit goes through every node, a node before its operands") {
    ...
    CHECK(seen
          == std::vector{proxima::Kind::Add, proxima::Kind::Integer,
                         proxima::Kind::Function, proxima::Kind::Symbol});
}
...
TEST_CASE("any_of stops at the first node that answers yes") {
    ...
    CHECK_FALSE(proxima::any_of(
        e, [](const Expr &node) { return node.is(proxima::Kind::Real); }));
}
...
TEST_CASE("transform rewrites bottom-up and shares what it leaves alone") {
    ...
    SUBCASE("each node once, operands first, parents rebuilt and normalised") {
        ...
        int calls = 0;
        const Expr doubled
            = proxima::transform(e, [&calls](const Expr &node) -> Expr {
                  ++calls;
                  return node.is(proxima::Kind::Integer)
                             ? Expr(node.integer_value()) * 2
                             : node;
              });
    ...
    SUBCASE("an untouched subtree is the same representation, not a copy") {
        ...
        CHECK(proxima::detail::same_representation(
            proxima::replace(e, Symbol("z"), Expr(1)), e));
    }
    ...
    SUBCASE("and nothing is evaluated") {
        const Expr result = proxima::transform(
            proxima::sin(Expr(x)), [&x](const Expr &node) -> Expr {
...
TEST_CASE("nodes is the tree as a range, in visit's order") {
    ...
    SUBCASE("and the standard algorithms apply") {
        ...
        CHECK(std::ranges::any_of(
            proxima::nodes(e), [](const Expr &n) { return n.is(Kind::Function); }));
        CHECK(std::ranges::count_if(proxima::nodes(e),
                                    [&](const Expr &n) { return n == Expr(x); })
              == 2);
        auto numbers = proxima::nodes(e) | std::views::filter([](const Expr &n) {
                           return n.is_number();
                       });
...
TEST_CASE("fold is the recursion the other walks are cases of") {
    ...
    CHECK(size
          == static_cast<std::size_t>(std::ranges::distance(proxima::nodes(e))));

    const auto depth
        = proxima::fold<int>(e, [](const Expr &, std::span<const int> depths) {
    ...
    SUBCASE("to bool, which a vector cannot hold as a span") {
        const auto mentions_y = [&](const Expr &tree) {
            return proxima::fold<bool>(
                tree, [&](const Expr &n, std::span<const bool> below) {
                    return n == Expr(y)
                           || std::ranges::any_of(below, [](bool b) { return b; });
                });
        };
        CHECK(mentions_y(e));
    ...
    SUBCASE("with match telling it the kind") {
        const auto calls
            = proxima::fold<int>(e, [](const Expr &n, std::span<const int> below) {
                  const int here
                      = n.match([](const proxima::node::Call &) { return 1; },
                                [](const auto &) { return 0; });
...
TEST_CASE("rewrite replaces what f says to, and shares the rest") {
    ...
    SUBCASE("changing nothing hands back the very same expression") {
        const Expr same = proxima::rewrite(
            e, [](const Expr &) { return std::optional<Expr>(); });
    ...
    SUBCASE("bottom-up, and normalised as it goes") {
        ...
        const Expr folded = proxima::rewrite(
            3 * Expr(x) + 2, [&](const Expr &n) -> std::optional<Expr> {
                if (n == Expr(x)) {
                    return Expr(2);
                }
                return std::nullopt;
